...
export default function MyListings() {
  ...
  // useEffect(
  //   function () {
  //     async function fetchRentalcounts() {
  //       try {
  //         if (
  //           ["available", "currentlyrented", "unavailable", "all"].includes(
  //             selectedType
  //           )
  //         ) {
  //           setIsLoading(true);
  //           const rentalCountsTemp = {};
  //           for (const car of myCars) {
  //             if (car.status !== "unavailable") {
  //               let res = await axios.get(
  //                 `http://localhost:8084/RentARide/bookings/rentalcount/${car.id}/active`
  //               );

  //               rentalCountsTemp[car.id] = { active: res.data };

  //               res = await axios.get(
  //                 `http://localhost:8084/RentARide/bookings/rentalcount/${car.id}/pending`
  //               );

  //               rentalCountsTemp[car.id] = {
  //                 ...rentalCountsTemp[car.id],
  //                 pending: res.data,
  //               };
  //             } else {
  ...
  //           setRentalCounts(rentalCountsTemp);
  //           setIsLoading(false);
  //         }
  //       } catch (error) {
  //         setIsLoading(false);
