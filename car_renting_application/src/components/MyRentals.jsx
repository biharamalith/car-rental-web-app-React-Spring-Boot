...
export default function MyRentals() {
  ...
  async function handleAcceptBooking(rentalId) {
    try {
      axios.put(`http://localhost:8084/RentARide/bookings/${rentalId}/accept`);
  ...
  function handleDeclineBooking(rentalId) {
    try {
      axios.put(`http://localhost:8084/RentARide/bookings/${rentalId}/decline`);
      setRerenderToggle((cur) => !cur);
    } catch (error) {
      throw new Error(error);
    }
  ...
  function handleCompleteRental(rentalId) {
    try {
      axios.put(
